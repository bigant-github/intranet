 ...
 * series of element nesting patterns to execute Rules that have been added prior to the start of parsing.  This package
 * was inspired by the
 ...
 * only be used within the context of a single thread at a time, and a call to <code>parse()</code> must be completed
 * before another can be initiated even from the same thread.</p>
 *
 * <p><strong>IMPLEMENTATION NOTE</strong> - A bug in Xerces 2.0.2 prevents
 * the support of XML schema. You need Xerces 2.1/2.3 and up to make this class working with XML schema</p>
 */
public class Digester extends DefaultHandler2 {
     ...
     * class loader used to load Digester itself, is used, based on the value of the
     ...
     * <code>org.xml.sax.XMLReader</code>. See <a href="http://www.saxproject.org/apidoc/xml/sax/package-summary.html#package-description">
    ...
    /**
     * Determine whether to use the Context ClassLoader (the one found by calling <code>Thread.currentThread().getContextClassLoader()</code>)
     * to resolve/load classes that are defined in various rules.  If not using Context ClassLoader, then the
     * class-loading defaults to using the calling-class' ClassLoader.
     *
     ...
     * <p>
     * FIX ME: there is a bug in JAXP/XERCES that prevent the use of a parser that contains a schema with a DTD.
     *
     ...
     * implementation. This maps <code>PUBLICID</code>'s to URLs (from which the resource will be loaded). A common use
     * case for this method is to register local URLs (possibly computed at runtime by a classloader) for DTDs. This
     * allows the performance advantage of using a local version without having to ensure every <code>SYSTEM</code> URI
     * on every processed xml document is local. This implementation provides only basic functionality. If more
     * sophisticated features are required, using {@link #setEntityResolver} to set a custom resolver is recommended.
     * </p><p>
