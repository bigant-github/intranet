...
public abstract class AbstractEndpoint<S> {
     ...
     * new connection. Until the limit is reached and thus the count cannot be incremented,  this value is more by 1
     * (the count of acceptors) than the actual count of connections that are being served.
     *
     * @return The count
     */
    ...
    /**
     * The maximum number of headers in a request that are allowed. 100 by default. A value of less than 0 means no
     * limit.
     */
    ...
    /**
     * Attributes provide a way for configuration to be passed to sub-components without the {@link
     * org.apache.coyote.ProtocolHandler} being aware of the properties available on those sub-components.
     */
    protected HashMap<String, Object> attributes = new HashMap<>();

    /**
     * Generic property setter called when a property for which a specific setter already exists within the {@link
     * org.apache.coyote.ProtocolHandler} needs to be made available to sub-components. The specific setter will call
     * this method to populate the attributes.
     *
    ...
    private void registerJmx(SSLHostConfig sslHostConfig) {
        ...
        for (SSLHostConfigCertificate sslHostConfigCert : sslHostConfig.getCertificates()) {
            ...
            try {
                sslCertOname = new ObjectName(domain + ":type=SSLHostConfigCertificate,ThreadPool=\"" + getName() + "\",Host=" + ObjectName.quote(sslHostConfig.getHostName()) + ",name=" + sslHostConfigCert.getType());
    ...
    /**
     * Provides a common approach for sub-classes to handle exceptions where a delay is required to prevent a Thread
     * from entering a tight loop which will consume CPU and may also trigger large amounts of logging. For example,
     * this can happen with the Acceptor thread if the ulimit for open files is reached.
     *
